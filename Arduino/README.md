...
the same movement and shooting actions. No Arduino sketch update is required for this migration.
...
Serial Monitor at **115200 baud** should show `512,512,0` near center; the last number becomes 1
while pressing fire. Close Serial Monitor/Plotter before Unity opens the port.
Both `x,y` and `x,y,fire` newline-terminated messages are accepted, with axes 0–1023 and fire 0/1.
...
controller choices are unaffected. Missing serial data releases movement/fire after 0.5 seconds;
...
for testing. This version does not change DungeonManager or implement Arduino press-to-join.
...
- `ArduinoInputDevice.cs`: the custom Input System layout (`move` Vector2 and `fire` button).
